...
#Final implementation of the SVDDense layer that compresses and prunes in batches.
class SVDDense(keras.layers.Layer):
    
    def __init__(self, units=32, lam_struct = 1, u_ort = 1000, u_sort = 1, lam_comp = 0.1, prune_threshold = 0.1  , layercount = 1, pruning_batch_size = 1, *args, **kwargs):
    ...
    def call(self, inputs):
        ...
        secondSmallest_batch = tf.reduce_sum(current_sigma[-self.pruning_batch_size*2:])
        ...
        self.add_loss(
            ...
            self.lam_comp * (smallest_batch) / self.layercount
            ) / self.layercount
        )
