...
#Oldver version of SVDDense. Compresses batched along with the final value, only attempts to prune the final value.

class SVDDense(keras.layers.Layer):
    
    def __init__(self, units=32, lam_struct = 1, u_ort = 1000, u_sort = 1, lam_comp = 0.1, prune_threshold = 0.1  , layercount = 1, compcount = 1, *args, **kwargs):
    ...
    def call(self, inputs):
        ...
        last_n_sigma = tf.reduce_sum(current_sigma[-self.compcount:])
        ...
        self.add_loss(
            ...
            self.lam_comp * (last_sigma + last_n_sigma) / self.layercount
            ) / self.layercount
        )
